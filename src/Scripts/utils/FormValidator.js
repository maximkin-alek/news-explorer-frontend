export class FormValidator {
  constructor(form, errorMessages, buttonClose) {
    ...
    this.addNewsFormValidator = this.addNewsFormValidator.bind(this);
  ...
  addNewsFormValidator(form) {
    const input = form.querySelector('input');
    const errorМessage = form.querySelector(`#error-${input.id}`);
    this._resetInputError(input, errorМessage);
    if (!input.value) {
      errorМessage.textContent = this._errorMessages.keyword;
      return false;
    }
    if (input.value.length < 2) {
      errorМessage.textContent = this._errorMessages.shortReq;
      return false;
    }
    return true;
  }
